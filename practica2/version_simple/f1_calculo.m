function f1_calculo(fTratada, fOriginal, indice_plot, nombre_colores)
...
figure(1); subplot(2, 3, indice_plot);imshow(fOriginal,[]);title(nombre_colores(indice_plot));

% Obtener los datos de area, perimetro y bounding box de cada objeto.
datos = regionprops(fEtiquetada, 'Area', 'Perimeter', 'BoundingBox', 'Circularity', 'Centroid');
...
for i = 1:length(datos)
    ...
    if (areaObj > 1500)
        ...
        fRecortada = fTratada(pFila_inicio:pFila_final, pColumna_inicio:pColumna_final);
        datosIndividualizados = regionprops(fRecortada, 'Centroid');
        elem = strel('disk', 1, 0);
        ...
        while(length(datosIndividualizados) < numObjetos)
            fRecortada = imerode(fRecortada, elem);
            datosIndividualizados = regionprops(fRecortada, 'Centroid', 'BoundingBox');
        end
